document.getElementById('search-form').addEventListener('submit', async function (event) {
    ...
    try {
        const response = await fetch('http://localhost:5096/api/scraper/scrape', {
            ...
            body: new URLSearchParams({ exactGroup }) // Теперь отправляем данные в формате form-data
        });
...
async function displayResults(data) {
    ...
    data.forEach(course => {
        const fullTeacherName = teacherMap.get(course.teacher?.toUpperCase()) || `${course.teacher}`;
